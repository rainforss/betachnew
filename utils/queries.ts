...
export const attachedComponentsQuery =
  "$filter=bsi_published ne false&$select=bsi_attachedcomponentid,bsi_name,bsi_description,bsi_ctabuttontext,bsi_ctabuttonlink,bsi_title,bsi_subtitle&$expand=bsi_AttachedComponent_bsi_ImageAsset_bsi_($select=bsi_cdnurl,bsi_name)";
...
export const dynamicsBlogSlugsQuery =
  "$filter=bsi_published ne false&$select=bsi_name,bsi_slug&$orderby=createdon asc";

export const dynamicsBlogAuthorsQuery =
  "$filter=bsi_published ne false&$select=bsi_name,bsi_slug&$orderby=bsi_name asc&$expand=bsi_Blog_bsi_BlogAuthor_bsi_BlogAuthor($select=bsi_name)";

export const dynamicsBlogCategoriesQuery =
  "$filter=bsi_published ne false&$select=bsi_name&$orderby=createdon asc&$expand=bsi_BlogCategory_bsi_Blog_bsi_Blog($select=bsi_name)";
...
export const generateBlogsODataQuery = (
  ...
  `${
    categorySlug
      ? `$filter=(bsi_BlogCategory_bsi_Blog_bsi_Blog/any(b:b/bsi_slug eq '${categorySlug}')) and bsi_published ne false&`
      : ""
  }${
    authorSlug
      ? `$filter=(bsi_Blog_bsi_BlogAuthor_bsi_BlogAuthor/any(b:b/bsi_slug eq '${authorSlug}')) and bsi_published ne false&`
      : ""
  }${
    blogSlug
      ? `$filter=bsi_slug eq '${blogSlug}' and bsi_published ne false&`
      : ""
